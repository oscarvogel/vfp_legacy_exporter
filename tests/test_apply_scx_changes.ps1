...
Assert-Contains $sandboxScript '\[string\]\$SandboxRoot\s*=\s*""' "sandbox root default is empty in param"
Assert-Contains $sandboxScript '\$scriptRoot\s*=\s*\$PSScriptRoot' "script root initialized after param"
Assert-Contains $sandboxScript '\$MyInvocation\.MyCommand\.Path' "script root fallback uses MyInvocation"
Assert-Contains $sandboxScript '\[string\]::IsNullOrWhiteSpace\(\$SandboxRoot\)' "empty sandbox root handled after param"
Assert-Contains $sandboxScript '\$repoRoot\s*=\s*Split-Path -Parent \$scriptRoot' "repo root based on resolved script root"
